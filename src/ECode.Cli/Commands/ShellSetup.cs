...
public static class ShellSetup
{
    ...
    public const string PowerShellEndMarker = "# <<< ecode setup <<<";
    public const string CmdBeginMarker = ":: >>> ecode setup >>>";
    public const string CmdEndMarker = ":: <<< ecode setup <<<";
    ...
    private static string CreatePowerShellBlock(string installDirectory)
    {
        ...
        {
            PowerShellBeginMarker,
            $"$ecodeBin = {quotedDirectory}",
            "if (($env:Path -split ';') -notcontains $ecodeBin) { $env:Path = \"$ecodeBin;$env:Path\" }",
            "Set-Alias -Name ecode -Value (Join-Path $ecodeBin 'ecode.exe') -Scope Global",
            PowerShellEndMarker,
        });
    }
    ...
    {
        var exePath = installDirectory + "\\ecode.exe";
        return string.Join(Environment.NewLine, new[]
        {
            CmdBeginMarker,
            $"doskey ecode=\"{exePath}\" $*",
            CmdEndMarker,
        });
    }
