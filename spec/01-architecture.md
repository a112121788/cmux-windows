...
> 项目代号：ECode（Windows 版），主程序集名 `ecode-app`，CLI `ecode`，守护进程 `ecode-daemon`。
...
ECode.sln
├── src/
│   ├── ECode/                # WPF 主程序（ecode-app.exe），UI / 控件 / 视图模型
...
| 进程 | 可执行文件 | 生命周期 | 主要职责 |
|---|---|---|---|
| **主应用** | `ecode-app.exe` | 用户启动 / 关闭 | WPF UI、用户交互、终端渲染、IPC 服务端（CLI 通道） |
