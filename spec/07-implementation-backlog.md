...
| ID | 标题 | 关联文件 | 验收 |
|---|---|---|---|
| `M1-B-01` | Surface 拖拽重排 | `Controls/SurfaceTabBar.xaml.cs` | 重排后 session 不丢 |
...
| `M4-B-05` | `console.list/clear`、`dialog.accept/dismiss`、`download.wait`、`highlight` | 同上 | 应做范围测试 |
| `M4-B-06` | `addinitscript / addscript / addstyle` | 同上 | 可做范围 |
...
| ID | 标题 | 关联文件 | 验收 |
|---|---|---|---|
| `M5-A-01` | 同一 `\\.\pipe\ecode` 上 v1/v2 协商（首行 JSON 判 v2） | `NamedPipeServer.cs`、`NamedPipeClient.cs` | v1 / v2 并存可运行 |
| `M5-A-02` | 短 ID 引用（`window:N / workspace:N / surface:N / pane:N`） | `ECode.Core/Models/ShortRef.cs` | UUID↔ref 双向解析 |
...
| ID | 标题 | 关联文件 | 验收 |
|---|---|---|---|
| `M5-B-01` | `WindowManagerService` | `ECode/Services/WindowManagerService.cs` | 多窗口独立生命周期 |
| `M5-B-02` | `window.list/current/focus/create/close` v2 API | `MainViewModel`、v2 协议层 | contract 测试 |
...
| ID | 标题 | 关联文件 | 验收 |
|---|---|---|---|
| `M6-B-01` | Velopack 集成（installer + feed） | `src/ECode.Updater`（新增）、`scripts/publish.ps1` | 新版本自动检测 |
...
| `M6-B-06` | `CHANGELOG.md` 自动生成（git-cliff 或 release-drafter） | `.github/release.yml` | release 触发自动更新 |
...
| ID | 标题 | 关联文件 | 验收 |
|---|---|---|---|
| `M7-A-01` | `docs/` 目录（mkdocs material 或 vitepress） | `docs/*` | 站点可构建 |
| `M7-A-02` | `installation.md` | `docs/installation.md` | 含 zip / Velopack / MSIX 三种安装方式 |
| `M7-A-03` | `getting-started.md` | `docs/getting-started.md` | 中文 / 英文 |
| `M7-A-04` | `custom-commands.md` | `docs/custom-commands.md` | 与 M5 后状态同步 |
| `M7-A-05` | `browser-api.md` | `docs/browser-api.md` | 与 M4 协议同步 |
...
7. `M2-A-03`（ResumeBinding 敏感环境剔除）
