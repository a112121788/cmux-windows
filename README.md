...
一款运行在 Windows 上的深色、键盘优先 SuperTerminal，面向多项目、多标签页、分屏、浏览器预览与脚本化自动化，底层使用 WPF + ConPTY 原生构建。
...
| 一个终端永远不够用                        | 命令行重度用户、长任务工作流              | **分屏（左右/上下）**             | `Ctrl+D` 向右分屏，`Ctrl+Shift+D` 向下分屏，`Ctrl+Alt+方向键` 聚焦面板 |
| 错过长任务的关键输出                     | 同时跑 dev server / 测试 / 日志的同学 | **OSC 通知 + 未读追踪**           | `Ctrl+I` 打开通知，`Ctrl+Shift+U` 跳到最新未读                           |
...
| 需要脚本/工具自动化                       | 集成方/自动化脚本                         | **命名管道 CLI API**（`ecode`） | `ecode notify`、`ecode workspace`、`ecode split`、`ecode status`     |
...
修改配置后可用 `Ctrl+Shift+,` 或 CLI `ecode reload-config` 热重载；如果命令面板已打开，会保留当前搜索词并刷新命令列表。
...
当前标签页的关闭按钮会常显；其他标签页在鼠标悬停时显示关闭按钮。
